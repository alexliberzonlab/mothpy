...
def moth_demo(dt=0.01, t_max=3, draw_iter_interval=20):
    """
    ...
    sim_region = models.Rectangle(0., -1., 2., 1.)
    #call moth model
    moth_model = models.moth_model(0,0,0,0,0,0,sim_region)
    # set up wind model
    wind_model = models.WindModel(wind_region, 21., 11.,noise_gain=0, u_av=1.,)
    # set up plume model
    plume_model = models.PlumeModel(sim_region, (0.1, 0., 0.), wind_model,
    ...
    #moth_model.moth_array taks in array from array generator as input, calculates moth position and adds that poisition(matrix addition) to the input
    #set _data then updates the plot image using the new matrix     
    draw_func = lambda: conc_im.set_data(
         moth_model.moth_array(plume_model,wind_model))
    # start simulation loop
    _simulation_loop(dt, t_max, time_text, draw_iter_interval, update_func,
                     draw_func)
    return fig
