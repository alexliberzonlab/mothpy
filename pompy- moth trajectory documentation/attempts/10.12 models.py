...
class moth_model(object):
    def __init__(self,x,y,u,v,alpha,D, sim_region):
        self.x = x
        self.y = y
        self.u = u
        self.v = v
    ...
    def moth_array(self, plume_model, wind_model):
        #set concetration array generator
        array_gen = processors.ConcentrationArrayGenerator(self.sim_region, 0.01, 500,
                                                       500, 1.)
        #set up conetration array
        conc_array = array_gen.generate_single_array(plume_model.puff_array).T
        #set wind at moth position
        wind_vel = wind_model.velocity_at_pos(self.x, self.y)
    ...
    def is_smelling(self,x,y,plume_matrix):
        """
