...
def concentration_array_demo(dt=0.01, t_max=5, draw_iter_interval=50):
    """
    ...
    draw_func = lambda: conc_im.set_data(
        array_gen.generate_single_array(plume_model.puff_array).T+o)
    # start simulation loop
    _simulation_loop(dt, t_max, time_text, draw_iter_interval, update_func,
                     draw_func)
    return fig
...
def moth_demo(dt=0.01, t_max=100, draw_iter_interval=20):
    """
    a copy of the concetration_array_demo with the moth actions integrated
    """
    #call moth model
    moth_model = models.Moth(0,0,0,0,0,0)
    # define simulation region
    wind_region = models.Rectangle(0., -2., 10., 2.)
    sim_region = models.Rectangle(0., -1., 2., 1.)
    # set up wind model
    ...
    # display initial concentration field as image
    #conc_array is the matrix into which the moth is to be inserted
    conc_array = array_gen.generate_single_array(plume_model.puff_array)
    im_extents = (sim_region.x_min, sim_region.x_max,
                  sim_region.y_min, sim_region.y_max)
    ...
    #conc_im.set data(...) draws unto conc.im
    #at this point i should switch plume model into plume + moth in the next line 
    draw_func = lambda: conc_im.set_data(
