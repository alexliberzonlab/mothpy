...
def moth_demo(dt=0.01, t_max=100, draw_iter_interval=20):
    #call moth model
    moth_model = Moth(0,0,0,0,0,0)
    #set up regions
    wind_region = models.Rectangle(0., -2., 10., 2.)
    sim_region = models.Rectangle(0., -1., 2., 1.)
    #get wind and concetration models
    wind_model = models.WindModel(sim_region, 21., 11.,
                                  u_av=1., Kx=2., Ky=2.)
    conc_model = concentration_array_demo.update(dt)

    def update_func(dt, t):
        wind_model.update(dt)
        concentration_array_demo.update(dt)
    fig, time_text = _set_up_figure('moth demo')

    def draw_func():
        conc_im.set_data(
            array_gen.generate_single_array(concentration_array_demo.conc_array).T[::-1, :])
        vf_plot.set_UVC(wind_model.velocity_field[:, :, 0].T,
                        wind_model.velocity_field[:, :, 1].T)
