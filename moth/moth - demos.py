...
def moth_demo(dt=0.01, t_max = 15, draw_iter_interval=20):
    ...
    *moth_model = models.moth_modular(sim_region, 450.0, 500.0,3,'carde1') y position is between 0 and 1000
    ...
    wind_region = models.Rectangle(0., 10.,-2., 2.)
    sim_region = models.Rectangle(0.,  4.,-1., 1.)
    #call moth model, set simulation region and starting position 
    moth_model = mothpy_models.moth_modular(sim_region, 450.0, 750.0,2, 'carde2',1)
    """
    nav,cast,wait
    benelli - 2,3,1
    Liberzon - 'alex',2,1
    ...
    # set up wind model
    wind_model = mothpy_models.WindModel(wind_region, 21, 11,noise_gain=0, u_av=1.,char_time =6,amplitude=0.3)
    # set up plume model
